...
def articles_to_tfidf(articles, stopwords):
	"""
	Returns TF-IDF matrix of the data
	Options for which data keys to use from articles within options file
	Also returns lists of ids, tags, and full data used
	"""
...
def calculate_grid(articles):
	...
	tfidf, ids, tags, data = articles_to_tfidf(articles, stopwords)
	...
	import numpy as np
	number_of_clusters = int(np.ceil(len(output)**0.3)+1)
	k_means = KMeans(init='k-means++', n_clusters=number_of_clusters, n_init=20)
	k_means.fit(tfidf)
